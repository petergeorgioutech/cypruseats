...
class Widget_Client_Logo extends Static_Grid {
	...
	public function get_name() {
		return 'golo-client-logo';
	}

	public function get_title() {
		return esc_html__( 'Client Logo', 'golo' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control( 'hover', [
			'label'   => esc_html__( 'Hover Type', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''          => esc_html__( 'None', 'golo' ),
				'grayscale' => esc_html__( 'Grayscale to normal', 'golo' ),
				'opacity'   => esc_html__( 'Opacity to normal', 'golo' ),
				'faded'     => esc_html__( 'Normal to opacity', 'golo' ),
			],
			'default' => '',
		] );
			...
			<a <?php $this->print_render_attribute_string( $item_link_key ); ?>>
				...
				<div class="image">
					<img src="<?php echo esc_url( $item['logo']['url'] ); ?>"
					     alt="<?php esc_attr_e( 'Client Logo', 'golo' ); ?>">
				</div>
	...
	protected function before_grid() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-client-logo' );
