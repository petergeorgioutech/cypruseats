...
class Widget_Google_Map extends Base {
	...
	public function get_name() {
		return 'golo-google-map';
	}

	public function get_title() {
		return esc_html__( 'Advanced Google Map', 'golo' );
	}
	...
	public function get_script_depends() {
		return [ 'gmap3', 'gmap-api', 'golo-widget-google-map' ];
	}
	...
	private function add_map_options_section() {
		...
		$this->add_control( 'style', [
			'label'   => esc_html__( 'Map Style', 'golo' ),
		...
		$this->add_control( 'type', [
			'label'   => esc_html__( 'Map Type', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__( 'Roadmap', 'golo' ),
				'satellite' => esc_html__( 'Satellite', 'golo' ),
				'hybrid'    => esc_html__( 'Hybrid', 'golo' ),
				'terrain'   => esc_html__( 'Terrain', 'golo' ),
			],
			'default' => 'roadmap',
		] );

		$this->add_control( 'zoom_level', [
			'label'       => esc_html__( 'Zoom Level', 'golo' ),
			'placeholder' => '12',
		...
		$this->add_control( 'scrollwheel', [
			'label'   => esc_html__( 'Mouse Scroll Wheel Zoom', 'golo' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'draggable', [
			'label'   => esc_html__( 'Draggable', 'golo' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'map_controls_heading', [
			'label'     => esc_html__( 'Controls', 'golo' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'zoom_control', [
			'label' => esc_html__( 'Zoom Control', 'golo' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'scale_control', [
			'label' => esc_html__( 'Scale Control', 'golo' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'fullscreen_control', [
			'label' => esc_html__( 'Fullscreen Control', 'golo' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'street_view_control', [
			'label' => esc_html__( 'Street View Control', 'golo' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function add_map_markers_section() {
		$this->start_controls_section( 'markers_section', [
			'label' => esc_html__( 'Markers', 'golo' ),
		] );

		$this->add_control( 'marker_style', [
			'label'       => esc_html__( 'Marker Style', 'golo' ),
			'description' => esc_html__( 'Select default style for all markers', 'golo' ),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__( 'Icon', 'golo' ),
				'signal' => esc_html__( 'Signal', 'golo' ),
			],
			'default'     => 'icon',
		] );

		$this->add_control( 'marker_icon', [
			'label'     => esc_html__( 'Marker Icon', 'golo' ),
			...
			'classes'   => 'golo-control-media-auto',
		] );
		...
		$repeater->start_controls_tab( 'marker_content_tab', [
			'label' => esc_html__( 'Marker', 'golo' ),
		] );

		$repeater->add_control( 'address', [
			'label'       => esc_html__( 'Address or Coordinate', 'golo' ),
		...
		$repeater->start_controls_tab( 'marker_popup_tab', [
			'label' => esc_html__( 'Popup', 'golo' ),
		] );

		$repeater->add_control( 'overlay_display', [
			'label'   => esc_html__( 'Display', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__( 'Hover', 'golo' ),
				'always' => esc_html__( 'Always', 'golo' ),
			],
			'default' => '',
		] );
		...
		$repeater->add_control( 'image', [
			...
			'description' => esc_html__( 'Choose an image that displays on left info box', 'golo' ),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'golo-control-media-auto',
		] );
		...
		$this->add_group_control( Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__( 'Popup Image Size', 'golo' ),
	...
	private function add_map_markers_style_section() {
		$this->start_controls_section( 'markers_popup_style_section', [
			'label' => esc_html__( 'Markers Popup', 'golo' ),
		...
		$this->add_responsive_control( 'markers_popup_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .golo-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .golo-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function get_overlay_value( $settings, $marker, $position ) {
		...
		$this->add_render_attribute( $item_key, 'class', [
			'golo-map-overlay',
			"golo-map-overlay-{$marker['overlay_display']}",
		...
		if ( $marker['title'] !== '' || $marker['content'] !== '' ) {
			$info_template .= '<div class="golo-map-overlay-template"><div class="golo-map-overlay-content"><div class="golo-map-overlay-info">';

			if ( isset( $marker['image'] ) ) {
				$size  = \Golo_Image::elementor_parse_image_size( $settings, '80x9999', 'popup_image_size' );
				$image = \Golo_Image::get_attachment_by_id( array(
					'id'   => $marker['image']['id'],
					'size' => $size,
				) );
	...
	private function get_map_style() {
		return [
			'no_label_bright_colors'  => esc_html__( 'No Label Bright Colors', 'golo' ),
			'grayscale'               => esc_html__( 'Grayscale', 'golo' ),
			'subtle_grayscale'        => esc_html__( 'Subtle Grayscale', 'golo' ),
			'apple_paps_esque'        => esc_html__( 'Apple Maps-esque', 'golo' ),
			'pale_dawn'               => esc_html__( 'Pale Dawn', 'golo' ),
			'midnight_commander'      => esc_html__( 'Midnight Commander', 'golo' ),
			'blue_water'              => esc_html__( 'Blue Water', 'golo' ),
			'retro'                   => esc_html__( 'Retro', 'golo' ),
			'paper'                   => esc_html__( 'Paper', 'golo' ),
			'ultra_light_with_labels' => esc_html__( 'Ultra Light with Labels', 'golo' ),
			'shades_of_grey'          => esc_html__( 'Shades Of Grey', 'golo' ),
			'easy_light'              => esc_html__( 'Easy Light', 'golo' ),
			'wy'                      => esc_html__( 'WY', 'golo' ),
		];
	}
	...
	private function get_default_marker_icon() {
		$icon_url = GOLO_ELEMENTOR_ASSETS . '/images/map-marker.png';
