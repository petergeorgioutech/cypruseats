...
class Widget_Circle_Progress_Chart extends Base {
	...
	public function get_name() {
		return 'golo-circle-progress-chart';
	}

	public function get_title() {
		return esc_html__( 'Circle Progress Chart', 'golo' );
	}
	...
	public function get_script_depends() {
		return [ 'golo-widget-circle-progress' ];
	}
	...
	private function add_chart_section() {
		$this->start_controls_section( 'chart_section', [
			'label' => esc_html__( 'Chart', 'golo' ),
		] );

		$this->add_control( 'number', [
			'label'   => esc_html__( 'Number', 'golo' ),
		...
		$this->add_control( 'size', [
			'label'       => esc_html__( 'Circle Size', 'golo' ),
			'description' => esc_html__( 'Controls the size of the pie chart circle.', 'golo' ),
			'type'        => Controls_Manager::NUMBER,
			'default'     => 180,
			'min'         => 100,
			'max'         => 1000,
		...
		$this->add_control( 'unit', [
			'label'       => esc_html__( 'Measuring unit', 'golo' ),
			'description' => esc_html__( 'Controls the unit of chart.', 'golo' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '%',
		] );

		$this->add_control( 'reverse', [
			'label' => esc_html__( 'Reverse animation and arc draw', 'golo' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'line_cap', [
			'label'   => esc_html__( 'Line Cap', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'butt'   => esc_html__( 'Butt', 'golo' ),
				'round'  => esc_html__( 'Round', 'golo' ),
		...
		$this->add_control( 'line_width', [
			'label'   => esc_html__( 'Line Width', 'golo' ),
			'type'    => Controls_Manager::NUMBER,
			'default' => 3,
			'min'     => 1,
			'max'     => 50,
		...
		$this->add_control( 'inner_content_type', [
			'label'   => esc_html__( 'Inner Content', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''     => esc_html__( 'Animate Number', 'golo' ),
				'text' => esc_html__( 'Custom Text', 'golo' ),
			],
			'default' => '',
		] );
	...
	private function add_chart_style_section() {
		...
		$this->add_control( 'bar_color', [
			'label'  => esc_html__( 'Bar Color', 'golo' ),
		...
		$this->add_control( 'track_color', [
			'label'   => esc_html__( 'Track Color', 'golo' ),
			'type'    => Controls_Manager::COLOR,
			'default' => '#ededed',
		] );

		$this->add_control( 'number_color', [
			'label'     => esc_html__( 'Number Color', 'golo' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-circle-progress-chart' );
