...
class Widget_Attribute_List extends Base {
	...
	public function get_name() {
		return 'golo-attribute-list';
	}

	public function get_title() {
		return esc_html__( 'Attribute List', 'golo' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'name'  => esc_html__( 'Attribute #1', 'golo' ),
					'value' => esc_html__( 'Value #1', 'golo' ),
				],
				[
					'name'  => esc_html__( 'Attribute #2', 'golo' ),
					'value' => esc_html__( 'Value #2', 'golo' ),
				],
			],
			'title_field' => '{{{ name }}}',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-attribute-list' );
					...
					<?php if ( $settings['items'] && count( $settings['items'] ) > 0 ) {
						foreach ( $settings['items'] as $key => $attribute ) {
							...
							<div class="item">
								...
								<div class="value"><?php echo wp_kses( $attribute['value'], 'golo-default' ); ?></div>
							</div>
							<?php
						}
					}
