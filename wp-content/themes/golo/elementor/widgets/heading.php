...
class Widget_Heading extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Heading', 'golo' );
	}
	...
	private function add_title_section() {
		...
		$this->add_control( 'title', [
			...
			'description' => esc_html__( 'Wrap any words with &lt;mark&gt;&lt;/mark&gt; tag to make them highlight.', 'golo' ),
		] );
		...
		$this->add_control( 'title_link_animate', [
			'label'        => esc_html__( 'Link Animate', 'golo' ),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''                  => esc_html__( 'None', 'golo' ),
				'animate-border'    => esc_html__( 'Animate Border', 'golo' ),
				'animate-border-02' => esc_html__( 'Animate Border 02', 'golo' ),
			],
			'default'      => '',
			'prefix_class' => 'golo-link-',
		] );
	...
	private function add_sub_title_section() {
		$this->start_controls_section( 'sub_title_section', [
			'label' => esc_html__( 'Secondary Heading', 'golo' ),
		] );
	...
	private function add_wrapper_style_section() {
		...
		$this->add_responsive_control( 'max_width', [
			...
			'selectors'      => [
				'{{WRAPPER}} .golo-modern-heading' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function add_title_style_section() {
		...
		$this->add_control( 'blend_mode', [
			'label'     => esc_html__( 'Blend Mode', 'golo' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
		...
		$this->add_control( 'highlight_heading', [
			'label'     => esc_html__( 'Highlight Words', 'golo' ),
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-modern-heading' );
		?>
