...
class Widget_List extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Icon List', 'golo' );
	}
	...
	private function add_list_section() {
		$this->start_controls_section( 'list_section', [
			'label' => esc_html__( 'Icon List', 'golo' ),
		] );
		...
		$this->add_control( 'style', [
			...
			'options'      => [
				...
				'icon-border' => esc_html__( 'Icon Border', 'golo' ),
			],
			'prefix_class' => 'golo-list-style-',
		] );
		...
		$this->add_control( 'layout', [
			...
			'options'      => [
				...
				'columns' => [
					'title' => esc_html__( 'Columns', 'golo' ),
					'icon'  => 'eicon-columns',
				],
			],
			'prefix_class' => 'golo-list-layout-',
		] );

		$this->add_control( 'icon', [
			'label'       => esc_html__( 'Default Icon', 'golo' ),
			'description' => esc_html__( 'Choose default icon for all items.', 'golo' ),
			'type'        => Controls_Manager::ICONS,
		] );

		$this->add_control( 'icon_vertical_alignment', [
			'label'                => esc_html__( 'Icon Alignment', 'golo' ),
			'type'                 => Controls_Manager::CHOOSE,
	...
	private function add_styling_section() {
		$this->start_controls_section( 'styling_section', [
			'label' => esc_html__( 'Styling', 'golo' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$this->add_control( 'items_vertical_spacing', [
			'label'      => esc_html__( 'Items Spacing', 'golo' ),
			...
			'selectors'  => [
				'{{WRAPPER}}.golo-list-layout-block .item + .item, {{WRAPPER}}.golo-list-layout-columns .item:nth-child(2) ~ .item' => 'margin-top: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}}.golo-list-layout-inline .item'                                                                             => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .golo-list' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function add_icon_style_section() {
		...
		$this->add_responsive_control( 'min_width', [
			'label'          => esc_html__( 'Min Width', 'golo' ),
			'type'           => Controls_Manager::SLIDER,
			'default'        => [
				'unit' => 'px',
			],
		...
		$this->add_control( 'icon_marker_color', [
			'label'     => esc_html__( 'Icon Marker', 'golo' ),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.golo-list-style-icon-border .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
		...
		$this->add_control( 'hover_icon_marker_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.golo-list-style-icon-border .link:hover .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-list' );

		$global_icon_html = '';
		if ( ! empty ( $settings['icon']['value'] ) ) {
			$global_icon_html = '<div class="golo-icon icon">' . $this->get_render_icon( $settings, $settings['icon'], [ 'aria-hidden' => 'true' ], false, 'icon' ) . '</div>';
		}
		?>
	...
	protected function _content_template() {
		...
		if ( '' !== settings.icon.value ) {
			...
			global_icon_html += '<div class="golo-icon icon">' + globalIconHTML.value + '</div>';
		}
		#>
		<div class="golo-list">
