...
class Widget_Twitter extends Base {
	...
	public function get_title() {
		return esc_html__( 'Twitter', 'golo' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control( 'consumer_key', [
			'label' => esc_html__( 'Consumer Key', 'golo' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'consumer_secret', [
			'label' => esc_html__( 'Consumer Secret', 'golo' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token', [
			'label' => esc_html__( 'Access Token', 'golo' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token_secret', [
			'label' => esc_html__( 'Access Token Secret', 'golo' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'username', [
			'label' => esc_html__( 'Twitter Username', 'golo' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'number_items', [
			'label'   => esc_html__( 'Number Items', 'golo' ),
		...
		$this->add_control( 'show_date', [
				'label'        => esc_html__( 'Show date', 'golo' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'golo' ),
				'label_off'    => esc_html__( 'No', 'golo' ),
				'return_value' => '1',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-twitter style-list' );
		?>
		...
		if ( $username !== '' && $consumer_key !== '' && $consumer_secret !== '' && $access_token !== '' && $access_token_secret !== '' && $number_items !== '' ) {
			...
			if ( false === $twitter_data || isset( $json['errors'] ) ) {
				...
				if ( ! $token ) {
					...
					$to_send     = golo_core_base_encode( $credentials );
