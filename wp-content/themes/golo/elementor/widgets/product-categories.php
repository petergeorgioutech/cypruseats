...
class Widget_Product_Categories extends Base {
	...
	public function get_name() {
		return 'golo-product-categories';
	}

	public function get_title() {
		return esc_html__( 'Product Categories', 'golo' );
	}
	...
	private function add_layout_section() {
		...
		$this->add_control( 'layout', [
			...
			'options' => [
				...
				'metro'   => esc_html__( 'Metro', 'golo' ),
			],
			'default' => 'grid',
		] );

		$this->add_responsive_control( 'zigzag_height', [
			'label'     => esc_html__( 'Zigzag Height', 'golo' ),
		...
		$this->add_control( 'zigzag_reversed', [
			'label'     => esc_html__( 'Zigzag Reversed?', 'golo' ),
		...
		$this->add_control( 'metro_image_ratio', [
			'label'     => esc_html__( 'Image Ratio', 'golo' ),
	...
	private function add_grid_section() {
		...
		$metro_layout_repeater->add_control( 'size', [
			'label'   => esc_html__( 'Item Size', 'golo' ),
			'type'    => Controls_Manager::SELECT,
			'default' => '1:1',
		...
		$this->add_control( 'grid_metro_layout', [
			'label'       => esc_html__( 'Metro Layout', 'golo' ),
	...
	private function add_query_section() {
		...
		$this->add_control( 'source', [
			...
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				''                      => esc_html__( 'Show All', 'golo' ),
				'by_id'                 => esc_html__( 'Manual Selection', 'golo' ),
				'by_parent'             => esc_html__( 'By Parent', 'golo' ),
				'current_subcategories' => esc_html__( 'Current Subcategories', 'golo' ),
			],
			'label_block' => true,
		] );
		...
		$this->add_control( 'categories', [
			'label'       => esc_html__( 'Categories', 'golo' ),
		...
		$parent_options = [ '0' => esc_html__( 'Only Top Level', 'golo' ) ] + $options;
		$this->add_control(
			'parent', [
			'label'     => esc_html__( 'Parent', 'golo' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '0',
		...
		$this->add_control( 'hide_empty', [
			'label'     => esc_html__( 'Hide Empty', 'golo' ),
			'type'      => Controls_Manager::SWITCHER,
			'default'   => 'yes',
			'label_on'  => 'Hide',
			'label_off' => 'Show',
		...
		$this->add_control( 'number', [
			'label'     => esc_html__( 'Categories Count', 'golo' ),
			'type'      => Controls_Manager::NUMBER,
			'default'   => '4',
			'condition' => [
				'source!' => 'by_id',
		...
		$this->add_control( 'orderby', [
			...
			'default'   => 'name',
			'options'   => [
				'name'        => esc_html__( 'Name', 'golo' ),
				'slug'        => esc_html__( 'Slug', 'golo' ),
				'description' => esc_html__( 'Description', 'golo' ),
				'count'       => esc_html__( 'Count', 'golo' ),
			],
			'condition' => [
				'source!' => 'by_id',
			],
		...
		$this->add_control( 'order', [
			'label'     => esc_html__( 'Order', 'golo' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => 'desc',
			'options'   => [
				'asc'  => esc_html__( 'ASC', 'golo' ),
				'desc' => esc_html__( 'DESC', 'golo' ),
			],
			'condition' => [
				'source!' => 'by_id',
			],
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid-wrapper', 'class', 'golo-grid-wrapper golo-product-categories' );

		$this->add_render_attribute( 'content-wrapper', 'class', 'golo-grid lazy-grid' );

		if ( 'metro' === $settings['layout'] ) {
			$this->add_render_attribute( 'grid-wrapper', 'class', 'golo-grid-metro' );
		}
	...
	private function print_grid() {
		$settings   = $this->get_settings_for_display();
		$image_size = \Golo_Image::elementor_parse_image_size( $settings, '300x300' );
		...
		foreach ( $this->terms as $term ) {
			...
			?>
			<div class="grid-item">
				<a href="<?php echo esc_url( $link ); ?>" class="product-cat-wrapper golo-box cat-link link-secret">
	...
	private function print_image( $thumbnail_id, $image_size ) {
		...
		<?php if ( ! empty( $thumbnail_id ) ) : ?>
			<?php \Golo_Image::the_attachment_by_id( array(
				'id'   => $thumbnail_id,
				'size' => $image_size,
			) ); ?>
		<?php else: ?>
