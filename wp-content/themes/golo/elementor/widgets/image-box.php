...
class Widget_Image_Box extends Base {
	...
	public function get_name() {
		return 'golo-image-box';
	}

	public function get_title() {
		return esc_html__( 'Image Box', 'golo' );
	}
	...
	private function add_image_box_section() {
		$this->start_controls_section( 'image_section', [
			'label' => esc_html__( 'Image Box', 'golo' ),
		] );
		...
		$this->add_control( 'description_text', [
			...
			'default'     => esc_html__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'golo' ),
	...
	private function add_box_style_section() {
		...
		$this->add_responsive_control( 'box_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .golo-box' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'box_max_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .golo-box' => 'max-width: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_group_control( Group_Control_Background::get_type(), [
			'name'     => 'box_hover',
			'selector' => '{{WRAPPER}} .golo-box:before',
		] );

		$this->add_group_control( Group_Control_Advanced_Border::get_type(), [
			'name'     => 'box_hover_border',
			'selector' => '{{WRAPPER}} .golo-box:hover',
		] );
	...
	private function add_image_style_section() {
		...
		$this->add_responsive_control( 'image_wrap_height', [
			...
			'selectors' => [
				'{{WRAPPER}} .golo-image' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		] );

		$this->add_responsive_control( 'image_space_top', [
			'label'     => esc_html__( 'Offset Top', 'golo' ),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'golo-image-box golo-box' );
	...
	protected function _content_template() {
		...
		?>
		<#
		view.addRenderAttribute( 'wrapper', 'class', 'golo-image-box golo-box' );
		...
		if ( settings.image.url ) {
			...
			imageHTML = '<div class="golo-image image"><img ' + view.getRenderAttributeString( 'image' ) + ' /></div>';
		}
		#>
	...
	private function print_description (array $settings) {
		...
		<div <?php $this->print_render_attribute_string('description_text'); ?>>
			<?php echo wp_kses($settings['description_text'], 'golo-default'); ?>
		</div>
		<?php
	}
}
