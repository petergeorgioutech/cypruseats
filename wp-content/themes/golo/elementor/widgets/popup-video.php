...
class Widget_Popup_Video extends Base {
	...
	public function get_name() {
		return 'golo-popup-video';
	}

	public function get_title() {
		return esc_html__( 'Popup Video', 'golo' );
	}
	...
	private function add_video_section() {
		...
		$this->add_control( 'type', [
			...
			'default' => 'poster',
			'options' => [
				'poster' => esc_html__( 'Poster', 'golo' ),
				'button' => esc_html__( 'Button', 'golo' ),
			],
		] );

		$this->add_control( 'video_url', [
			'label'       => esc_html__( 'Video Url', 'golo' ),
			'description' => esc_html__( 'Input Youtube video url or Vimeo video url. For e.g: "https://www.youtube.com/watch?v=XHOmBV4js_E"', 'golo' ),
		...
		$this->add_control( 'video_text', [
			'label'         => esc_html__( 'Video Text', 'golo' ),
			'type'          => Controls_Manager::TEXT,
			'label  _block' => true,
			'condition'     => [
				'type' => 'button',
		...
		$this->add_control( 'video_text_animate', [
			'label'        => esc_html__( 'Text Animate', 'golo' ),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''             => esc_html__( 'None', 'golo' ),
				'animate-line' => esc_html__( 'Animate Line', 'golo' ),
			],
			'default'      => '',
			'prefix_class' => 'golo-text-',
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
			],
		...
		$this->add_control( 'position', [
			...
			'prefix_class' => 'golo-popup-video-icon-position-',
			'toggle'       => false,
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
		...
		$this->add_control( 'poster', [
			'label'     => esc_html__( 'Poster Image', 'golo' ),
		...
		$this->add_control( 'button_type', [
			'label'     => esc_html__( 'Button Type', 'golo' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '',
			'options'   => [
		...
		$this->add_control( 'button_image', [
			'label'     => esc_html__( 'Button Image', 'golo' ),
			'type'      => Controls_Manager::MEDIA,
			'default'   => [
	...
	private function add_image_style_section() {
		...
		$this->add_responsive_control( 'image_border_width', [
			...
			'selectors' => [
				'{{WRAPPER}} .golo-image' => 'border-width: {{SIZE}}{{UNIT}}',
			],
		] );
		...
		$this->add_responsive_control( 'image_border_radius', [
			...
			'selectors'  => [
				'{{WRAPPER}} .golo-image' => 'border-radius: {{SIZE}}{{UNIT}}',
			],
		] );
		...
		$this->add_control( 'image_border_color', [
			...
			'selectors' => [
				'{{WRAPPER}} .golo-image' => 'border-color: {{VALUE}};',
			],
		] );

		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'name'     => 'image_box_shadow',
			'selector' => '{{WRAPPER}} .golo-image',
		] );
		...
		$this->add_control( 'hover_image_border_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .video-link:hover .golo-image' => 'border-color: {{VALUE}};',
			],
		] );

		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'name'     => 'hover_image_box_shadow',
			'selector' => '{{WRAPPER}} .video-link:hover .golo-image',
		] );
	...
	private function add_button_style_section() {
		...
		$this->add_responsive_control( 'button_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .golo-popup-video-icon-play .video-play-icon' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}}; font-size: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}} .golo-popup-video-image-play .video-play img'            => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'button_text_color', [
			'label'     => esc_html__( 'Icon Color', 'golo' ),
			'type'      => Controls_Manager::COLOR,
			'default'   => '',
			'selectors' => [
	...
	protected function render() {
		...
		if ( ! empty( $settings['button_type'] ) && 'image' === $settings['button_type'] ) {
			$this->add_render_attribute( 'wrapper', 'class', 'golo-popup-video-image-play' );
		} else {
			$this->add_render_attribute( 'wrapper', 'class', 'golo-popup-video-icon-play' );
		}

		$this->add_render_attribute( 'link', 'class', 'video-link golo-box link-secret' );
	...
	private function get_default_play_icon() {
		$icon_url = GOLO_ELEMENTOR_ASSETS . '/images/video-play-light.png';
