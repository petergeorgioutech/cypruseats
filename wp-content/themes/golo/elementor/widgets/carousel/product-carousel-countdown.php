...
class Widget_Product_Carousel_Countdonw extends Posts_Carousel_Base {

	public function get_name() {
		return 'golo-product-carousel-countdown';
	}

	public function get_title() {
		return esc_html__( 'Product Carousel Countdown', 'golo' );
	}
	...
	private function add_countdown_section() {
		$this->start_controls_section( 'countdown_section', [
			'label' => esc_html__( 'Countdown', 'golo' ),
		] );
		...
		$this->add_control( 'countdown_type', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'due_date' => esc_html__( 'Due Date', 'golo' ),
				'daily'    => esc_html__( 'Daily', 'golo' ),
			],
			'default' => 'daily',
		] );

		$this->add_control( 'due_date', [
			'label'       => esc_html__( 'Due Date', 'golo' ),
			'type'        => Controls_Manager::DATE_TIME,
			'default'     => gmdate( 'Y-m-d H:i', strtotime( '+1 month' ) + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) ),
			/* translators: %s: Time zone. */
			'description' => sprintf( __( 'Date set according to your timezone: %s.', 'golo' ), Utils::get_timezone_string() ),
			'condition'   => [
		...
		$this->add_control( 'countdown_title', [
			...
			'default' => esc_html__( 'Daily Deal Of The Day', 'golo' ),
		] );
	...
	public function before_slider() {
		...
		echo '<div class="golo-product-carousel-countdown">';
	...
	private function print_countdown() {
		...
		$days_text    = isset( $days ) && $days !== '' ? $days : esc_html__( 'Days', 'golo' );
		$hours_text   = isset( $hours ) && $hours !== '' ? $hours : esc_html__( 'Hours', 'golo' );
		$minutes_text = isset( $minutes ) && $minutes !== '' ? $minutes : esc_html__( 'Minutes', 'golo' );
		$seconds_text = isset( $seconds ) && $seconds !== '' ? $seconds : esc_html__( 'Seconds', 'golo' );
		...
		<div class="product-countdown-header">
			...
			<div class="countdown-wrap">
				<div class="countdown-label"><?php esc_html_e( 'End in:', 'golo' ); ?></div>
