...
class Widget_Testimonial_Carousel extends Static_Carousel {
	...
	public function get_name() {
		return 'golo-testimonial';
	}

	public function get_title() {
		return esc_html__( 'Testimonial Carousel', 'golo' );
	}
	...
	private function add_layout_section() {
		...
		$this->add_control( 'style', [
			...
			'prefix_class' => 'golo-testimonial-style-',
		] );
		...
		$this->add_control( 'layout', [
			...
			'options'      => [
				'image-inline'  => esc_html__( 'Image Inline', 'golo' ),
				'image-stacked' => esc_html__( 'Image Stacked', 'golo' ),
				'image-top'     => esc_html__( 'Image Top Overlap', 'golo' ),
				'image-top-02'  => esc_html__( 'Image Top', 'golo' ),
				'image-above'   => esc_html__( 'Image Above', 'golo' ),
				'image-left'    => esc_html__( 'Image Left', 'golo' ),
			],
			'render_type'  => 'template',
			'prefix_class' => 'layout-',
		] );
		...
		$this->add_control( 'image_position', [
			'label'        => esc_html__( 'Info Position', 'golo' ),
			'type'         => Controls_Manager::CHOOSE,
			'label_block'  => false,
			'default'      => 'below',
			'options'      => [
				'above'  => [
					'title' => esc_html__( 'Above', 'golo' ),
					'icon'  => 'eicon-v-align-top',
				],
				'below'  => [
					'title' => esc_html__( 'Below', 'golo' ),
		...
		$this->add_control( 'cite_layout', [
			'label'        => esc_html__( 'Cite Layout', 'golo' ),
			'label_block'  => false,
			'type'         => Controls_Manager::CHOOSE,
			'default'      => 'block',
			'options'      => [
				'block'  => [
					'title' => esc_html__( 'Default', 'golo' ),
					'icon'  => 'eicon-editor-list-ul',
				],
				'inline' => [
					'title' => esc_html__( 'Inline', 'golo' ),
					'icon'  => 'eicon-ellipsis-h',
				],
			],
			'prefix_class' => 'golo-testimonial-cite-layout-',
		] );
	...
	protected function add_repeater_controls( Repeater $repeater ) {
		...
		$repeater->add_control( 'name', [
			'label'   => esc_html__( 'Name', 'golo' ),
			'type'    => Controls_Manager::TEXT,
			'default' => esc_html__( 'John Doe', 'golo' ),
		] );

		$repeater->add_control( 'position', [
			'label'   => esc_html__( 'Position', 'golo' ),
			'type'    => Controls_Manager::TEXT,
			'default' => esc_html__( 'CEO', 'golo' ),
		] );

		$repeater->add_control( 'rating', [
			'label' => esc_html__( 'Rating', 'golo' ),
	...
	protected function get_repeater_defaults() {
		...
		return [
			[
				'content'  => esc_html__( 'I am slide content. Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'golo' ),
				'name'     => esc_html__( 'John Doe', 'golo' ),
				'position' => esc_html__( 'Web Design', 'golo' ),
	...
	protected function update_slider_settings( $settings, $slider_settings ) {
		if ( 'image-above' === $settings['layout'] ) {
			$slider_settings['class'][]            = 'golo-main-swiper';
	...
	private function print_layout_image_above() {
		...
		<?php if ( $slide['content'] ) : ?>
			<div class="content">
				<div class="text">
					<?php echo wp_kses( $slide['content'], 'golo-default' ); ?>
				</div>
			</div>
		<?php endif; ?>
	...
	protected function after_slider() {
		...
		$this->add_render_attribute( '_wrapper', 'class', 'golo-swiper-linked-yes' );

		$testimonial_thumbs_template = '';

		foreach ( $settings['slides'] as $slide ) :
			if ( $slide['image']['url'] ) :
				$testimonial_thumbs_template .= '<div class="swiper-slide"><div class="post-thumbnail"><div class="image">' . \Golo_Image::get_elementor_attachment( [
						'settings'       => $slide,
						'image_size_key' => 'image_size',
					] ) . '</div></div></div>';
			endif;
		...
		?>
		<div class="golo-swiper golo-slider-widget golo-testimonial-pagination style-01 golo-thumbs-swiper"
		     data-lg-items="3"
		     data-lg-gutter="30"
