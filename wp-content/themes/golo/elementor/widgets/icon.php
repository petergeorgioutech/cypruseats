...
class Widget_Icon extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Icon', 'golo' );
	}
	...
	private function add_icon_style_section() {
		...
		$this->add_group_control( Group_Control_Text_Gradient::get_type(), [
			'name'     => 'hover_icon',
			'selector' => '{{WRAPPER}} .golo-icon:hover .icon',
		] );
		...
		$this->add_responsive_control( 'icon_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .golo-icon, {{WRAPPER}} .golo-icon-view' => 'font-size: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_group_control( Group_Control_Background::get_type(), [
			...
			'selector' => '{{WRAPPER}} .golo-icon:hover .golo-icon-view',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'box', 'class', 'golo-icon' );
		$box_tag = 'div';
	...
	protected function _content_template() {
		...
		<#
		view.addRenderAttribute( 'box', 'class', 'golo-icon' );
		var box_tag = 'div';
		...
		<{{{ box_tag }}} {{{ view.getRenderAttributeString( 'box' ) }}}>
			<div class="golo-icon-wrap">
				...
				<# if ( 'bubble' == settings.view ) { #>
					<div class="golo-icon-view second"></div>
				<# } #>
