	...
	function( $ ) {
		...
		$.fn.GoloGridQuery = function() {
			...
			function initFilterCount() {
				if ( ! $el.children( '.golo-grid-filter' ).data( 'filter-counter' ) ) {
					return;
				}
			...
			function handlerFilter() {
				$el.children( '.golo-grid-filter' ).on( 'click', '.btn-filter', function( e ) {
			...
			function handlerPagination() {
				...
				if ( $el.data( 'pagination' ) === 'load-more' ) {
					$el.children( '.golo-grid-pagination' )
					   .find( '.golo-load-more-button' )
					   .on( 'click', function( e ) {
						   e.preventDefault();
			...
			function handlerQuery( reset ) {
				...
				if ( infiniteLoader === true ) {
					loader = $el.find( '.golo-infinite-loader' );
				} else {
					loader = $grid.children( '.golo-grid-loader' );
				}
				...
				setTimeout( function() {
					...
					$.ajax( {
						...
						success: function( results ) {
							...
							$el.trigger( 'GoloQueryEnd', [ $el, $newItems ] );
			...
			function handlerQueryEnd() {
				...
				if ( foundPosts <= (
					...
					if ( $el.data( 'pagination' ) === 'load-more-alt' ) {
						...
						$el.children( '.golo-grid-pagination' ).hide();
					}

					$el.children( '.golo-grid-messages' ).show( 1 );
					setTimeout( function() {
						$el.children( '.golo-grid-messages' ).remove();
					}, 5000 );
				} else {
					if ( $el.data( 'pagination' ) === 'load-more-alt' ) {
						var loadMoreBtn = $( $el.data( 'pagination-custom-button-id' ) );
						...
						$el.children( '.golo-grid-pagination' ).show();
						$el.children( '.golo-grid-pagination' ).find( '.golo-load-more-button' ).show();
					}
			...
			return this.each( function() {
				...
				$queryInput = $el.find( '.golo-query-input' ).first();

				initFilterCount();
				handlerFilter();
				handlerPagination();

				$el.on( 'GoloBeginQuery', function() {
					// Reset to first page.
