...
/*--- TODO ----------------------------------------------------
 * Add better support for the 'gofollow' class in clicktracking when advert has multiple urls.
 * Stats, stats, stats! - per Advertiser stats overview. Geo targeting stats perhaps.
-------------------------------------------------------------*/

/*--- AdRotate values ---------------------------------------*/
define("ADROTATE_DISPLAY", '5.8.22 Professional');
...
if(is_admin()) {
	adrotate_check_config(); // has to be early
}
...
if(is_admin()) {
	...
	add_action('upgrader_process_complete', 'adrotate_update_finished', 10, 2); // Clean up after updates
...
function adrotate_dashboard() {
	...
	$adrotate_page = $adrotate_adverts = $adrotate_groups = $adrotate_schedules = $adrotate_statistics = $adrotate_advertisers = $adrotate_media = $adrotate_support = $adrotate_settings = '';
	...
	$adrotate_adverts = add_submenu_page('adrotate', 'AdRotate Pro · '.__('Manage Adverts', 'adrotate-pro'), __('Manage Adverts', 'adrotate-pro'), 'adrotate_ad_manage', 'adrotate', 'adrotate_manage_adverts');
	...
	$adrotate_media = add_submenu_page('adrotate', 'AdRotate Pro · '.__('Manage Files', 'adrotate-pro'), __('Manage Files', 'adrotate-pro'), 'adrotate_ad_manage', 'adrotate-media', 'adrotate_manage_media');
	if($adrotate_config['enable_advertisers'] == 'Y') {
		$adrotate_advertisers = add_submenu_page('adrotate', 'AdRotate Pro · '.__('Manage Advertisers', 'adrotate-pro'), __('Manage Advertisers', 'adrotate-pro'), 'adrotate_advertiser_manage', 'adrotate-advertisers', 'adrotate_manage_advertisers');
	}
//	$adrotate_swap = add_submenu_page('adrotate', 'AdRotate Pro · AdRotate Swap', __('Manage Campaign', 'adrotate-pro'), 'adrotate_ad_manage', 'adrotate-swap', 'adrotate_swap');
	if($adrotate_config['stats'] == 1) {
	...
	$adrotate_support = add_submenu_page('adrotate', 'AdRotate Pro · '.__('Premium Support', 'adrotate-pro'), __('Premium Support', 'adrotate-pro'), 'adrotate_ad_manage', 'adrotate-support', 'adrotate_support');
	...
	// Add help tabs
	add_action('load-'.$adrotate_page, 'adrotate_dashboard_help');
...
//	add_action('load-'.$adrotate_swap, 'adrotate_dashboard_help');
	add_action('load-'.$adrotate_statistics, 'adrotate_dashboard_help');
	add_action('load-'.$adrotate_advertisers, 'adrotate_dashboard_help');
	add_action('load-'.$adrotate_support, 'adrotate_dashboard_help');
...
function adrotate_options() {
	...
	<div class="wrap">
		...
		if($active_tab == 'general') {
			...
			$updatecheck = wp_next_scheduled('adrotate_update_check');
		...
		<?php if($status > 0) adrotate_status($status); ?>
	  	...
	  	<form name="settings" id="post" method="post" action="admin.php?page=adrotate-network-license">
			...
			<table class="form-table">
				...
				<?php } ?>
				<?php if($adrotate_hide_license == 0) { ?>
				<tr>
				...
				<tr>
					<th valign="top"><?php _e('Hide License', 'adrotate-pro'); ?></th>
					<td>
						<input type="checkbox" name="adrotate_license_hide" <?php echo ($adrotate_activate['status'] == 1) ? 'disabled' : ''; ?> /> <span class="description"><?php _e('If you have installed AdRotate Pro for a client or in a multisite setup and want to hide the License Key, Email address from them.', 'adrotate-pro'); ?></span>
					</td>
				</tr>
