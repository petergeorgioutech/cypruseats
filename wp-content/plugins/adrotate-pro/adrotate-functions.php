...
/*-------------------------------------------------------------
 ...
 Purpose:   Set up a session for Geo Targeting
 Since:		5.6.1
-------------------------------------------------------------*/
...
function adrotate_geolocation() {
	...
	if(!isset($_SESSION['adrotate-geo']) AND adrotate_is_human()) {
		...
		if($adrotate_config['enable_geo'] == 5) { // AdRotate Geo
			if(!get_transient('adrotate_api_banned')) {
				...
				if($lookups < 1 AND !get_transient('adrotate_geo_reset')) {
					...
					$oneminute = gmdate('U') + 60;
					set_transient('adrotate_geo_reset', 1, $oneminute);
				}
...
function adrotate_evaluate_ad($ad_id) {
	...
	$schedules = wp_cache_get($cachekey);
	if(false === $schedules) {
		$schedules = $wpdb->get_results("SELECT `{$wpdb->prefix}adrotate_schedule`.* FROM `{$wpdb->prefix}adrotate_schedule`, `{$wpdb->prefix}adrotate_linkmeta` WHERE `schedule` = `{$wpdb->prefix}adrotate_schedule`.`id` AND `ad` = {$ad_id} ORDER BY `starttime` ASC;");
		wp_cache_set($cachekey, $schedules, '', 300);
	}
	...
	if(
		...
		OR ((!preg_match_all('/<(a)[^>](.*?)>/i', stripslashes(htmlspecialchars_decode($ad->bannercode, ENT_QUOTES)), $things) OR preg_match_all('/<(ins|script|embed|iframe)[^>](.*?)>/i', stripslashes(htmlspecialchars_decode($ad->bannercode, ENT_QUOTES)), $things)) AND ($ad->tracker == 'Y' OR $ad->tracker == 'C')) // Clicks active but no valid link/tag present
...
function adrotate_status($status, $args = null) {
	...
	switch($status) {
		...
		case '512' :
			echo '<div id="message" class="error"><p>'. __('There was an error unzipping the file. Please try again later.', 'adrotate-pro') .'</p></div>';
		break;
