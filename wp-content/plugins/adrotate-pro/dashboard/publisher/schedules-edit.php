...
if($edit_schedule) {
	...
	if($schedule_edit_id AND $edit_schedule->name != '') {
		...
		if($edit_schedule->maximpressions < 1000 AND ($edit_schedule->spread == 'Y' OR $edit_schedule->spread_all == 'Y')) 
			echo '<div class="error"><p>'. __('Maximum Impressions is set very low. This may cause adverts to intermittently not show. Raising the impression limit or shortening the schedule time period may fix this.', 'adrotate-pro').'</p></div>';
