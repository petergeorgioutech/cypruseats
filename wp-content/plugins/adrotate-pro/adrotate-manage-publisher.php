		...
		if(current_user_can('adrotate_ad_manage')) {
			if(strlen($portability) == 0) {
				...
				if(count($srcset) > 0) {
					...
					$scrset_html = ' srcset="'.implode(", ", $srcset).'"';
					$sizes_html = ' sizes="'.implode(", ", $sizes).'"';
				} else {
					$scrset_html = $sizes_html = '';
				}
	...
	if(wp_verify_nonce($_POST['adrotate_nonce'], 'adrotate_save_media')) {
		if(current_user_can('adrotate_ad_manage')) {
			...
			if($_FILES["adrotate_image"]["size"] > 0 AND $_FILES["adrotate_image"]["size"] <= 512000) {
				...
				if(
					in_array($file_extension, array("jpg", "jpeg", "gif", "png", "svg", "html", "htm", "js", "zip"))
