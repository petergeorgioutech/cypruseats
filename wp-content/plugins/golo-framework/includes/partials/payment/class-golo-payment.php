...
if (!class_exists('Golo_Payment')) {
    /**
     * Class Golo_Payment
     */
    class Golo_Payment
    {
        protected $golo_invoice;
        protected $golo_package;
        protected $golo_trans_log;
        ...
        {
            $this->golo_package = new Golo_Package();
            $this->golo_invoice = new Golo_Invoice();
            $this->golo_trans_log = new Golo_Trans_Log();
        }
        ...
        {
            require_once(GOLO_PLUGIN_DIR . 'includes/partials/payment/stripe-php/init.php');
            $stripe_secret_key = golo_get_option('stripe_secret_key');
            $stripe_publishable_key = golo_get_option('stripe_publishable_key');
            ...
            $currency_code = golo_get_option('currency_code', 'USD');
            $package_price = $package_price * 100;
            $payment_completed_link = golo_get_permalink('payment_completed');
            $stripe_processor_link = add_query_arg(array('payment_method' => 2), $payment_completed_link);
            wp_enqueue_script('stripe-checkout');
            wp_localize_script('stripe-checkout','golo_stripe_vars',array(
                'golo_stripe_per_package' => array(
                    ...
                    'params' => array(
                        ...
                        'zipCode' => true,
                        'billingAddress' => true,
                        'name' => esc_html__( 'Pay with Credit Card', 'golo-framework' ),
                        'description' => wp_kses_post(sprintf(__('%s Package Payment', 'golo-framework'),$package_name))
                    )
                )
            ));
            ?>
            <form class="golo-stripe-form" action="<?php echo esc_url($stripe_processor_link)?>" method="post" id="golo_stripe_per_package">
                <button class="golo-stripe-button" style="display: none !important;"></button>
        ...
        {
            $client_id = golo_get_option('paypal_client_id');
            $secret_key = golo_get_option('paypal_client_secret_key');
        ...
        {
            check_ajax_referer('golo_payment_ajax_nonce', 'golo_security_payment');
            global $current_user;
            wp_get_current_user();
            $user_id = $current_user->ID;
            ...
            $currency = golo_get_option('currency_code', 'USD');
            $payment_description = $package_name . ' ' . esc_html__('Membership payment on ', 'golo-framework') . $blogInfo;
            $is_paypal_live = golo_get_option('paypal_api');
            ...
            $dash_profile_link = golo_get_permalink('dashboard');
            ...
            $payment['transactions'][0]['item_list']['items'][] = array(
                'quantity' => '1',
                'name' => esc_html__('Payment Package', 'golo-framework'),
                'price' => $package_price,
                'currency' => $currency,
                'sku' => $package_name . ' ' . esc_html__('Payment Package', 'golo-framework'),
            );
            ...
            update_user_meta($user_id, GOLO_METABOX_PREFIX . 'paypal_transfer', $output);
        ...
        public function wire_transfer_per_package_ajax()
        {
            ...
            $package_id = $_POST['package_id'];
            $package_id = intval($package_id);
            $total_price = get_post_meta($package_id, GOLO_METABOX_PREFIX . 'package_price', true);
            $total_price = golo_get_format_money($total_price);
            $payment_method = 'Wire_Transfer';
            // insert invoice
            $invoice_id = $this->golo_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 0);
            $args = array(
             ...
             * Send email
             * */
            golo_send_email($user_email, 'mail_new_wire_transfer', $args);
            golo_send_email($admin_email, 'admin_mail_new_wire_transfer', $args);
            $payment_completed_link = golo_get_permalink('payment_completed');
        ...
        public function free_package_ajax()
        {
            ...
            // insert invoice
            $invoice_id = $this->golo_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 1);

            $this->golo_package->insert_user_package($user_id, $package_id);
            update_user_meta($user_id, GOLO_METABOX_PREFIX . 'free_package', 'yes');
            $payment_completed_link = golo_get_permalink('payment_completed');
        ...
        {
            require_once(GOLO_PLUGIN_DIR . 'includes/partials/payment/stripe-php/init.php');
            $paid_submission_type = golo_get_option('paid_submission_type');
            $current_user = wp_get_current_user();
            $user_id = $current_user->ID;
            $user_email = $current_user->user_email;
            $admin_email = get_bloginfo('admin_email');
            ...
            try {
                $token = isset($_POST['stripeToken']) ? golo_clean(wp_unslash($_POST['stripeToken'])) : '';
                ...
                if ($payment_Status == "succeeded") {
                    if ($paid_submission_type == 'per_package') {
                        ...
                        $this->golo_package->insert_user_package($user_id, $package_id);
                        $this->golo_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 1, $paymentId, $payerId);
                        $args = array();
                        golo_send_email($user_email, 'mail_activated_package', $args);
                    }
                } else {
                    $message = esc_html__('Transaction failed', 'golo-framework');
                    if ($paid_submission_type == 'per_listing') {
                        //Payment Stripe listing
                        $place_id = absint(wp_unslash($_POST['place_id']));

                        if ($payment_for == 3) {
                            $this->golo_trans_log->insert_trans_log('Upgrade_To_Featured', $place_id, $user_id, 3, $payment_method, 0, $paymentId, $payerId,0, $message);
                        } else {
                            if ($payment_for == 2) {
                                $this->golo_trans_log->insert_trans_log('Listing_With_Featured', $place_id, $user_id, 2, $payment_method, 0, $paymentId, $payerId,0, $message);
                            } else {
                                $this->golo_trans_log->insert_trans_log('Listing', $place_id, $user_id, 1, $payment_method, 0, $paymentId, $payerId,0, $message);
                            }
                        }
                    } else if ($paid_submission_type == 'per_package') {
                        //Payment Stripe package
                        $package_id = absint(wp_unslash($_POST['package_id']));
                        $this->golo_trans_log->insert_trans_log('Package', $package_id, $user_id, 0, $payment_method, 0, $paymentId, $payerId,0, $message);
                    }

                    $error = '<div class="alert alert-error" role="alert">' . wp_kses_post(__('<strong>Error!</strong> Transaction failed', 'golo-framework')) . '</div>';
                    echo wp_kses_post($error);
                }
            } catch (Exception $e) {
                $error = '<div class="alert alert-error" role="alert"><strong>'. esc_html__( 'Error!', 'golo-framework' ) .' </strong> ' . $e->getMessage() . '</div>';
                echo wp_kses_post($error);
            }
        }
        ...
        public function paypal_payment_completed()
        {
            ...
            try {
                if (isset($_GET['token']) && isset($_GET['PayerID'])) {
                    $payerId = wp_kses( golo_clean(wp_unslash($_GET['PayerID'])), $allowed_html);
                    $paymentId = wp_kses( golo_clean(wp_unslash($_GET['paymentId'])), $allowed_html);
                    $transfered_data = get_user_meta($user_id, GOLO_METABOX_PREFIX . 'paypal_transfer', true);
                    ...
                    delete_user_meta($user_id, GOLO_METABOX_PREFIX . 'paypal_transfer');
                    if ($json_response['state'] == 'approved') {
                        ...
                        if ($paid_submission_type == 'per_listing') {
                            ...
                            $place_id = $transfered_data['place_id'];
                            if ($payment_for == 3) {
                                $this->golo_trans_log->insert_trans_log('Upgrade_To_Featured', $place_id, $user_id, 3, $payment_method, 0, $paymentId, $payerId,0, $message);
                            } else {
                                if ($payment_for == 2) {
                        ...
                        $error = '<div class="alert alert-error" role="alert">' . sprintf(__('<strong>Error!</strong> Transaction failed', 'golo-framework')) . '</div>';
                        print $error;
                    }
                }
            } catch (Exception $e) {
