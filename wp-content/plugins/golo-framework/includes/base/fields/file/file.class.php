	...
	class GLF_Field_File extends GLF_Field
	{
		function enqueue() {
			...
			wp_localize_script(GLF_BASE_RESOURCE_PREFIX . 'file', 'sfFileFieldMeta', array(
				'title'   => esc_html__('Select File', 'golo-framework'),
				'button'  => esc_html__('Use these files', 'golo-framework')
			));
		}
		...
		function render_content($content_args = '')
		{
			...
			$remove_text = esc_html__('Remove','golo-framework');
			?>
			<div class="glf-field-file-inner" data-remove-text="<?php echo esc_attr($remove_text); ?>"
				...
				<?php foreach ($field_value_arr as $file_id): ?>
					...
					<div class="glf-file-item" data-file-id="<?php echo esc_attr($file_id); ?>">
						...
						<div class="glf-file-info">
							<a class="glf-file-title" href="<?php echo esc_url(get_edit_post_link($file_id)); ?>" target="_blank"><?php echo esc_html($file_meta->post_title); ?></a>
							<div class="glf-file-name"><?php echo esc_html(wp_basename($file_meta->guid)); ?></div>
							<div class="glf-file-action">
								<span class="glf-file-remove"><span class="dashicons dashicons-no-alt"></span>  <?php echo esc_html($remove_text) ?></span>
							</div>
						</div>
					</div>
				<?php endforeach;?>
				<div class="glf-file-add">
					<button class="button" type="button"><?php esc_html_e('+ Add File','golo-framework'); ?></button>
				</div>
			</div>
			<?php
		}
