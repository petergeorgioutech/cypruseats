			...
			foreach ($meta_configs as $config) {
				...
				if (in_array($taxonomy, $taxonomies)) {
					...
					<?php if (!empty($config['name'])): ?>
						<h3 class="glf-taxonomy-title"><?php echo esc_html($config['name']); ?></h3>
					<?php endif;?>
					<?php
...
// 			        $the_query->the_post();
// 			        update_post_meta( get_the_ID(), 'golo-place_booking_banner', '' );
// 			        update_post_meta( get_the_ID(), 'golo-place_images', '' );
