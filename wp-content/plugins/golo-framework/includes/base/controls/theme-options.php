...
if (!class_exists('GLF_Theme_Options')) {
	class GLF_Theme_Options
	{
		...
		public function binder_page()
		{
			...
			$page = golo_clean(wp_unslash($_GET['page']));
		...
		public function save_theme_options()
		{
			...
			try {
				...
				$message = esc_html__('Settings saved', 'golo-framework');
				add_settings_error('gf-options', 'update-options', $message, 'updated');
			} catch (Exception $e) {
				$message = esc_html__('There has been an error when update options', 'golo-framework');
