    ...
    class Golo_Plugins 
    {
        ...
        public static function get_plugin_action( $plugin ) {
            ...
            if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
                ...
                $actions = sprintf(
                    __( '<a href="%1$s" title="Install %2$s">Install</a>', 'golo-framework' ),
                    esc_url(
                        wp_nonce_url(
                            add_query_arg(
                                array(
                ...
                // Display update link.
                $actions = sprintf(
                    __( '<a href="%1$s" title="Update %2$s">Update</a>', 'golo-framework' ),
                    wp_nonce_url(
                        add_query_arg(
                            array(
                                'page'         => rawurlencode( TGM_Plugin_Activation::$instance->menu ),
                ...
                // Display Active link.
                $actions = sprintf(
                    __( '<a href="%1$s" title="Activate %2$s" data-slug="%3$s" data-source="%4$s" data-plugin-action="activate-plugin" data-nonce="%5$s" class="golo-plugin-action plugin-activate">Activate</a>', 'golo-framework' ),
                    '#',
                    $plugin['name'],
                    $plugin['slug'],
                    $plugin['file_path'],
                ...
                $actions = sprintf(
                    __( '<a href="%1$s" title="Deactivate %2$s" data-slug="%3$s" data-source="%4$s" data-plugin-action="deactivate-plugin" data-nonce="%5$s" class="golo-plugin-action plugin-deactivate">Deactivate</a>', 'golo-framework' ),
                    '#',
                    $plugin['name'],
                    $plugin['slug'],
                    $plugin['file_path'],
        ...
        public function process_plugin_actions() {
            ...
            if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
                wp_send_json_error( esc_html__( 'TGM_Plugin_Activation does not exist', 'golo-framework' ) );
            }
            ...
            if ( empty( $source ) ) {
                wp_send_json_error( esc_html__( 'Installation package not available.', 'golo-framework' ) );
            }
