<?php
/**
 * Class for declaring the content importer used in the Golo Framework plugin
 *
 * @package Golo_Framework
 */

/**
 * Class Golo_Content_Importer
 */
class Golo_Content_Importer {
	...
	/**
	 * The instance of the Golo_Import_Logger class.
	 *
	 * @var Golo_Import_Logger
	 */
	public $logger;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the Golo_Importer instance.
		$this->tmi = Golo_Importer::instance();
	}
	...
	public function import_content( $import_file_path ) {
		...
		set_time_limit( apply_filters( 'golo_time_limit_for_demo_data_import', 300 ) );
		...
		if ( ! apply_filters( 'golo_regenerate_thumbnails', false ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'golo_time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			Golo_Import_Logger::append_to_file(
				esc_html__( 'New AJAX call!', 'golo-framework' ) . PHP_EOL . $message,
	...
	private function set_current_importer_data() {
		...
		set_transient( 'golo_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}
}