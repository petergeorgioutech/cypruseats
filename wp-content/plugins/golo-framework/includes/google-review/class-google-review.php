<?php
function goloGetReviews($placeid) {

    $google_review_api_key  = golo_get_option('google_review_api_key', 'AIzaSyBvgFc-WtkrUrAhTE_7an-OkBNlqReY_Bc' );
    ...
    if ($data['error_message']) {
        ...
        echo '<p class="error-box">' . 'GOLO msg: Place review file exists? ' . file_exists($option['google_maps_review_cid'] . '.json') . '</p>';
    } else {
        ...
        if (!empty($reviews)) {
            ...
            $html .= '<h3 class="entry-title">' . esc_html__('Google Reviews', 'golo-framework') . '</h3>';
            $html .= '</div>';
            ...
            $icon = GOLO_PLUGIN_URL . 'assets/images/google.png';
            ...
            if (isset($option['show_stars_in_headline']) and $option['show_stars_in_headline'] == true) {
                for ($i=1; $i <= $data['result']['rating']; ++$i) $html .= '<i class="la la-star check"></i>';
                if (isset($option['show_blank_star_till_5']) and $option['show_blank_star_till_5'] == true) for ($i=1; $i <= 5-floor($data['result']['rating']); ++$i) $html .= '<i class="la la-star"></i>';
            }
            $html .= '<span class="number-star">' . $data['result']['rating'] . '</span>';
            $html .= '</span>'; // rate-star
            if ($data['result']['user_ratings_total'] == 1) {
                $html .= '<span class="count">' . $data['result']['user_ratings_total'] . esc_html__(' review', 'golo-framework') . '</span>';
            } else {
                $html .= '<span class="count">' . $data['result']['user_ratings_total'] . esc_html__(' reviews', 'golo-framework') . '</span>';
            }
            
            $html .= '</div>'; // google-reviews-info
            $html .= '<div class="google-reviews-add">';
            $html .= '<a href="https://search.google.com/local/writereview?placeid=' . $option['google_maps_review_cid'] . '" target="_Blank"><i class="las la-comments"></i>' . esc_html__('Add Review', 'golo-framework') . '</a>';
            ...
            foreach ($reviews as $key => $review) {
                ...
                for ($i=1; $i <= $review['rating']; ++$i) $html .= '<i class="la la-star check"></i>';
                if (isset($option['show_blank_star_till_5']) and $option['show_blank_star_till_5'] == true) for ($i=1; $i <= 5-$review['rating']; ++$i) $html .= '<i class="la la-star"></i>';
