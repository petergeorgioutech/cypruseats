 ...
 */
abstract class Golo_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		add_action( 'save_post', array( $this, 'golo_widget_cache' ) );
		add_action( 'deleted_post', array( $this, 'golo_widget_cache' ) );
		add_action( 'switch_theme', array( $this, 'golo_widget_cache' ) );
	...
	public function enqueue_resources(){
		// base
		wp_enqueue_style(GOLO_PLUGIN_PREFIX . 'widget', GOLO_PLUGIN_URL . '/modules/widgets/assets/css/widget.css', array(), false, 'all');
		wp_enqueue_script(GOLO_PLUGIN_PREFIX . 'widget', GOLO_PLUGIN_URL . '/modules/widgets/assets/js/widget.js',array(), false, true);
	}
	 ...
	 * @return void
	 */
	public function golo_widget_cache() {
			...
			switch ( $setting['type'] ) {
                ...
                case "multi-select" :
                    ...
                    <p>
                        <label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html($setting['label']); ?></label>
                        <input name="<?php echo esc_attr($this->get_field_name( $key )); ?>" type="hidden" value="<?php echo esc_attr( $value ); ?>" />
                        <select multiple="multiple" class="widefat golo-select2" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" data-value="<?php echo esc_attr($value) ?>">
                            <?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
				...
				case "icon":
					...
					<div>
						...
						<div>
							...
							<input style="float: right"  title="<?php echo esc_html__('Click to browse icon','golo-framework') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_html__('Browse...','golo-framework') ?>" />
				...
				case "image":
					...
					<div style="margin: 13px 0">
						...
						<div class="widget-image-field">
							...
							<button style="margin-top: 10px" title="<?php echo esc_html__('Click to browse image','golo-framework') ?>" class="browse-image button-secondary" type="button"><?php echo esc_html__('Upload image','golo-framework') ?></button>
						</div>
					</div>
